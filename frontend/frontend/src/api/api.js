...
export const superAdminAPI = {
  ...
  // Analytics
  analyticsOverview: () => api.get('/analytics/overview'),
  analyticsMrr: (periodo = 12) => api.get(`/analytics/mrr`, { params: { periodo } }),
  analyticsConversao: (periodo = 12) => api.get(`/analytics/conversao`, { params: { periodo } }),
  analyticsChurn: (periodo = 12) => api.get(`/analytics/churn`, { params: { periodo } }),
  analyticsFunil: () => api.get('/analytics/funil'),

  // Webhook Logs
  webhookLogs: (params) => api.get('/webhooks/logs', { params }),
  webhookReprocess: (id) => api.post(`/webhooks/reprocess/${id}`),
};
