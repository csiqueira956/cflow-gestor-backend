// SuperAdminPanel v2 - Icons updated + Analytics
...
const SuperAdminPanel = () => {
  ...
  const [analyticsMrr, setAnalyticsMrr] = useState([]);
  const [analyticsConversao, setAnalyticsConversao] = useState([]);
  const [analyticsChurn, setAnalyticsChurn] = useState([]);
  const [analyticsFunil, setAnalyticsFunil] = useState([]);
  const [loadingAnalytics, setLoadingAnalytics] = useState(false);
  const [analyticsPeriodo, setAnalyticsPeriodo] = useState(12);

  // Webhook logs states
  const [showWebhookLogsModal, setShowWebhookLogsModal] = useState(false);
  const [webhookLogs, setWebhookLogs] = useState([]);
  const [loadingWebhooks, setLoadingWebhooks] = useState(false);
  const [webhookFilter, setWebhookFilter] = useState({ status: 'all', event_type: 'all' });
  ...
  useEffect(() => {
    const handleTabChange = (event) => {
      ...
      if (['dashboard', 'empresas', 'planos', 'monitoramento', 'leads', 'analytics'].includes(tab)) {
        setActiveTab(tab);
      }
    };
  ...
  // Carregar analytics quando a tab mudar
  useEffect(() => {
    if (activeTab === 'analytics') {
      carregarAnalytics();
    }
  }, [activeTab, analyticsPeriodo]);
  ...
  const carregarMrrHistory = async () => {
    try {
      setLoadingMrr(true);
      const response = await superAdminAPI.mrrHistory(12);
      setMrrHistory(response.data.history || []);
    } catch {
      console.error('Erro ao carregar histórico MRR');
    } finally {
      setLoadingMrr(false);
    }
  };
  ...
  const carregarAnalytics = async () => {
    try {
      ...
      const [overviewRes, mrrRes, conversaoRes, churnRes, funilRes] = await Promise.all([
        superAdminAPI.analyticsOverview(),
        superAdminAPI.analyticsMrr(analyticsPeriodo),
        superAdminAPI.analyticsConversao(analyticsPeriodo),
        superAdminAPI.analyticsChurn(analyticsPeriodo),
        superAdminAPI.analyticsFunil(),
      ]);
      setAnalyticsOverview(overviewRes.data.data || overviewRes.data);
      setAnalyticsMrr(mrrRes.data.data || []);
      setAnalyticsConversao(conversaoRes.data.data || []);
      setAnalyticsChurn(churnRes.data.data || []);
      setAnalyticsFunil(funilRes.data.data || []);
    } catch (error) {
  ...
  // Carregar webhook logs
  const carregarWebhookLogs = async () => {
    try {
      setLoadingWebhooks(true);
      const response = await superAdminAPI.webhookLogs({
        status: webhookFilter.status !== 'all' ? webhookFilter.status : undefined,
        event_type: webhookFilter.event_type !== 'all' ? webhookFilter.event_type : undefined,
        limit: 100,
      });
  ...
  // Reprocessar webhook
  const handleReprocessWebhook = async (id) => {
    try {
      await superAdminAPI.webhookReprocess(id);
      toast.success('Webhook enviado para reprocessamento');
      carregarWebhookLogs();
    } catch {
  ...
  const handleExport = async (tipo) => {
    try {
      ...
      switch (tipo) {
        case 'empresas':
          response = await superAdminAPI.exportEmpresas();
          filename = `empresas_${new Date().toISOString().split('T')[0]}.csv`;
          break;
        case 'usuarios':
          response = await superAdminAPI.exportUsuarios();
          filename = `usuarios_${new Date().toISOString().split('T')[0]}.csv`;
          break;
        case 'financeiro':
          response = await superAdminAPI.exportFinanceiro();
      ...
      toast.success(`Export de ${tipo} realizado com sucesso!`);
    } catch {
      toast.error(`Erro ao exportar ${tipo}`);
    } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-all duration-300 ${isExpanded ? 'ml-64' : 'ml-20'}`} style={{ maxWidth: '1472px' }}>
        <div className="card">
            ...
            <div className="space-y-6">
              {loadingDashboard && !dashboardData ? (
                ...
                <>
                  ...
                  {/* Quarta linha - Gráfico MRR e Export */}
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border p-6">
                      <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                        ...
                        MRR - Receita Mensal Recorrente (12 meses)
                      </h3>
                      {loadingMrr ? (
                        <div className="flex justify-center py-12">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex items-end justify-between gap-2 h-48">
                            {mrrHistory.map((item, index) => {
                              const maxMrr = Math.max(...mrrHistory.map(m => m.mrr || 0));
                              const percentage = maxMrr > 0 ? ((item.mrr || 0) / maxMrr) * 100 : 0;
                              return (
                          ...
                          {/* Legenda com valores */}
                          <div className="flex flex-wrap gap-4 pt-4 border-t justify-center">
                            <div className="text-center">
                              <p className="text-xs text-gray-500">Atual</p>
                              <p className="font-bold text-purple-600">{formatCurrency(mrrHistory[mrrHistory.length - 1]?.mrr || 0)}</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs text-gray-500">Média</p>
                              <p className="font-bold text-gray-700">
                                {formatCurrency(mrrHistory.reduce((acc, m) => acc + (m.mrr || 0), 0) / mrrHistory.length)}
                              </p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs text-gray-500">Total Acumulado</p>
                              <p className="font-bold text-emerald-600">{formatCurrency(mrrHistory[mrrHistory.length - 1]?.acumulado || 0)}</p>
                            </div>
                          </div>
                        </div>
                      ) : (
                        <div className="text-center py-12 text-gray-500">
                          ...
                          <p>Nenhum dado de MRR disponível</p>
                        </div>
                      )}
                    </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-xs text-gray-500">Lista completa de empresas</p>
                            </div>
                          </div>
                          {exportando.empresas ? (
                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
                          ) : (
                            <span className="text-xs bg-blue-200 text-blue-700 px-2 py-1 rounded font-medium">CSV</span>
                          )}
                        </button>

                        <button
                          onClick={() => handleExport('usuarios')}
                          disabled={exportando.usuarios}
                          className="w-full flex items-center justify-between p-3 bg-purple-50 hover:bg-purple-100 rounded-lg border border-purple-200 transition-colors disabled:opacity-50"
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-xs text-gray-500">Todos os usuários do sistema</p>
                            </div>
                          </div>
                          {exportando.usuarios ? (
                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-purple-600"></div>
                          ) : (
                            <span className="text-xs bg-purple-200 text-purple-700 px-2 py-1 rounded font-medium">CSV</span>
                          )}
                        </button>

                        <button
                          onClick={() => handleExport('financeiro')}
                          disabled={exportando.financeiro}
                          className="w-full flex items-center justify-between p-3 bg-emerald-50 hover:bg-emerald-100 rounded-lg border border-emerald-200 transition-colors disabled:opacity-50"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center">
                              <svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-xs text-gray-500">Relatório financeiro completo</p>
                            </div>
                          </div>
                          {exportando.financeiro ? (
                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-emerald-600"></div>
                          ) : (
                            <span className="text-xs bg-emerald-200 text-emerald-700 px-2 py-1 rounded font-medium">CSV</span>
                          )}
                        </button>
                      </div>
                      <p className="text-xs text-gray-400 mt-4 text-center">
                        Os arquivos CSV podem ser abertos no Excel
                      </p>
                    </div>
                  </div>
                </>
          ...
          {activeTab === 'analytics' && (
            <div className="space-y-6">
              {/* Header com Período e Webhook Logs */}
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <h2 className="text-lg font-medium text-gray-900">Analytics SaaS</h2>
                  <p className="text-sm text-gray-500">Métricas e indicadores de desempenho</p>
                </div>
                <div className="flex gap-3">
                  <select
                    value={analyticsPeriodo}
                    onChange={(e) => setAnalyticsPeriodo(parseInt(e.target.value))}
                    className="input-field py-2 text-sm"
                  >
                ...
                <>
                  ...
                  {analyticsOverview && (
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                      <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        <div className="text-2xl font-bold text-blue-600">{analyticsOverview.total_empresas || 0}</div>
                        <div className="text-sm text-blue-800">Total Empresas</div>
                      </div>
                      <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                        <div className="text-2xl font-bold text-green-600">{analyticsOverview.empresas_ativas || 0}</div>
                        <div className="text-sm text-green-800">Ativas</div>
                      </div>
                      <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                        <div className="text-2xl font-bold text-yellow-600">{analyticsOverview.empresas_trial || 0}</div>
                        <div className="text-sm text-yellow-800">Trial</div>
                      </div>
                      <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                        <div className="text-2xl font-bold text-purple-600">{formatCurrency(analyticsOverview.mrr || 0)}</div>
                        <div className="text-sm text-purple-800">MRR</div>
                      </div>
                      <div className="bg-cyan-50 rounded-lg p-4 border border-cyan-200">
                        <div className="text-2xl font-bold text-cyan-600">{analyticsOverview.novos_no_mes || 0}</div>
                        <div className="text-sm text-cyan-800">Novos no Mês</div>
                      </div>
                      <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                        <div className="text-2xl font-bold text-red-600">{analyticsOverview.churn_no_mes || 0}</div>
                        <div className="text-sm text-red-800">Churn Mês</div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white rounded-lg border p-6">
                      <h3 className="text-lg font-medium text-gray-900 mb-4">Evolução do MRR</h3>
                      {analyticsMrr.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart data={analyticsMrr}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="mes" tick={{ fontSize: 12 }} />
                            <YAxis tick={{ fontSize: 12 }} tickFormatter={(v) => `R$${(v/1000).toFixed(0)}k`} />
                            <Tooltip formatter={(v) => [`R$ ${v.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, 'MRR']} />
                            <Legend />
                            <Line type="monotone" dataKey="mrr" stroke="#0a4ee4" strokeWidth={2} name="MRR" dot={{ fill: '#0a4ee4' }} />
                          </LineChart>
                        </ResponsiveContainer>
                      ) : (
                    ...
                    <div className="bg-white rounded-lg border p-6">
                      ...
                      {analyticsConversao.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <BarChart data={analyticsConversao}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="mes" tick={{ fontSize: 12 }} />
                            <YAxis tick={{ fontSize: 12 }} />
                            <Tooltip />
                            <Legend />
                            <Bar dataKey="trials" fill="#f59e0b" name="Trials" />
                    ...
                    {/* Gráfico de Churn */}
                    <div className="bg-white rounded-lg border p-6">
                      <h3 className="text-lg font-medium text-gray-900 mb-4">Taxa de Churn</h3>
                      {analyticsChurn.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart data={analyticsChurn}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="mes" tick={{ fontSize: 12 }} />
                            <YAxis tick={{ fontSize: 12 }} tickFormatter={(v) => `${v}%`} />
                            <Tooltip formatter={(v) => [`${v}%`, 'Taxa de Churn']} />
                            <Legend />
                            <Line type="monotone" dataKey="taxa_churn" stroke="#ef4444" strokeWidth={2} name="Churn %" dot={{ fill: '#ef4444' }} />
                          </LineChart>
                        </ResponsiveContainer>
                      ) : (
                        <div className="h-[300px] flex items-center justify-center text-gray-500">
                    ...
                    {/* Gráfico de Funil */}
                    <div className="bg-white rounded-lg border p-6">
                      <h3 className="text-lg font-medium text-gray-900 mb-4">Funil de Assinaturas</h3>
                      {analyticsFunil.length > 0 ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <PieChart>
                            <Pie
                              data={analyticsFunil}
                            ...
                            >
                              {analyticsFunil.map((entry, index) => {
                                const COLORS = ['#0a4ee4', '#10b981', '#f59e0b', '#ef4444', '#6b7280'];
      ...
      {/* Modal de Webhook Logs */}
      {showWebhookLogsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full p-6 max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold text-gray-900">Webhook Logs (Asaas)</h2>
              <button
                onClick={() => {
                  setShowWebhookLogsModal(false);
            ...
            <div className="flex gap-3 mb-4">
              <select
                ...
                onChange={(e) => {
                  setWebhookFilter(prev => ({ ...prev, status: e.target.value }));
                  setTimeout(carregarWebhookLogs, 100);
                }}
                className="input-field py-2 text-sm"
              >
              ...
              <select
                value={webhookFilter.event_type}
                onChange={(e) => {
                  setWebhookFilter(prev => ({ ...prev, event_type: e.target.value }));
                  setTimeout(carregarWebhookLogs, 100);
                }}
                className="input-field py-2 text-sm"
              >
                ...
                <option value="PAYMENT_UPDATED">Pagamento Atualizado</option>
              </select>
              <button
                onClick={carregarWebhookLogs}
                disabled={loadingWebhooks}
                className="btn-secondary flex items-center gap-2"
              >
                <svg className={`w-4 h-4 ${loadingWebhooks ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                Atualizar
              </button>
            ...
            <div className="flex-1 overflow-y-auto">
                    ...
                    {webhookLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <button
                              onClick={() => setSelectedWebhook(selectedWebhook?.id === log.id ? null : log)}
                              className="text-blue-600 hover:text-blue-800 text-sm"
                            >
                              {selectedWebhook?.id === log.id ? 'Fechar' : 'Ver Payload'}
                            </button>
                            {log.status === 'failed' && (
                              <button
                                onClick={() => handleReprocessWebhook(log.id)}
              ...
              {/* Payload do webhook selecionado */}
              {selectedWebhook && (
