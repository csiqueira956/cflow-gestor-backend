...
app.get('/api/dashboard/estatisticas', async (req, res) => {
  try {
    ...
    if (role === 'vendedor' || role === 'gerente') {
      vendasPorEquipeQuery = `
        SELECT e.id as equipe_id, e.nome as equipe_nome,
          COALESCE(SUM(CASE WHEN c.valor_carta ~ '^[0-9]+(\\.[0-9]+)?$' THEN CAST(c.valor_carta AS NUMERIC) ELSE 0 END), 0) as total_vendido,
    ...
    const ticketQuery = await pool.query(`
      SELECT AVG(CASE WHEN valor_carta ~ '^[0-9]+(\\.[0-9]+)?$' THEN CAST(valor_carta AS NUMERIC) ELSE NULL END) as ticket_medio
    ...
    // 5. Pipeline Value
    const pipelineNeg = await pool.query(`SELECT COALESCE(SUM(CASE WHEN valor_carta ~ '^[0-9]+(\\.[0-9]+)?$' THEN CAST(valor_carta AS NUMERIC) ELSE 0 END), 0) as valor, COUNT(*) as qtd FROM clientes WHERE etapa = 'negociacao' AND valor_carta IS NOT NULL AND valor_carta != '' AND company_id = $1`, [companyId]);
    const pipelineProp = await pool.query(`SELECT COALESCE(SUM(CASE WHEN valor_carta ~ '^[0-9]+(\\.[0-9]+)?$' THEN CAST(valor_carta AS NUMERIC) ELSE 0 END), 0) as valor, COUNT(*) as qtd FROM clientes WHERE etapa = 'proposta_enviada' AND valor_carta IS NOT NULL AND valor_carta != '' AND company_id = $1`, [companyId]);
    ...
    const rankingResult = await pool.query(`
      SELECT u.id, u.nome, e.nome as equipe_nome, COUNT(c.id) as total_vendas,
        COALESCE(SUM(CASE WHEN c.valor_carta ~ '^[0-9]+(\\.[0-9]+)?$' THEN CAST(c.valor_carta AS NUMERIC) ELSE 0 END), 0) as total_valor
...
app.get('/api/metas', async (req, res) => {
  try {
    ...
    if (req.query.equipe_id) {
      ...
      values.push(req.query.equipe_id);
      paramCount++;
    }
    if (req.query.mes_referencia) {
      query += ` AND m.mes_referencia = $${paramCount}`;
      values.push(req.query.mes_referencia);
      paramCount++;
    }
    if (req.query.status) {
    ...
    const result = await pool.query(query, values);
    res.json({ data: { metas: result.rows } });
  } catch (error) {
...
app.get('/api/metas/:id', async (req, res) => {
  try {
    ...
    res.json({ data: { meta: result.rows[0] } });
  } catch (error) {
  ...
  try {
    ...
    const result = await pool.query(`
      INSERT INTO metas (
        ...
        valor_meta, mes_referencia, status, company_id
      )
    ...
    `, [
      ...
      descricao || null,
      tipo,
      tipo === 'vendedor' ? vendedor_id : null,
    ...
    res.status(201).json({ data: { meta: result.rows[0] }, message: 'Meta criada com sucesso' });
  } catch (error) {
...
app.put('/api/metas/:id', async (req, res) => {
  try {
    ...
    const checkResult = await pool.query(
      'SELECT id FROM metas WHERE id = $1 AND company_id = $2',
      [id, companyId]
    );
    ...
    res.json({ data: { meta: result.rows[0] }, message: 'Meta atualizada com sucesso' });
  } catch (error) {
...
app.delete('/api/metas/:id', async (req, res) => {
  try {
    ...
    // Verificar se meta existe
    const checkResult = await pool.query(
      'SELECT id, titulo FROM metas WHERE id = $1 AND company_id = $2',
      [id, companyId]
    );
    ...
    await pool.query('DELETE FROM metas WHERE id = $1 AND company_id = $2', [id, companyId]);
