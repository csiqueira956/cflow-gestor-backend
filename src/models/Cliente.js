...
class Cliente {
  ...
  // Listar clientes (filtrado por company_id)
  static async list(company_id, vendedorId = null) {
    let query = `
  ...
  // Listar clientes por lista de vendedores (para gerentes, filtrado por company_id)
  static async listByVendedores(company_id, vendedoresIds) {
    if (!vendedoresIds || vendedoresIds.length === 0) {
      return [];
    }

    // Criar placeholders para o IN clause: $2, $3, $4, etc ($1 é company_id)
    ...
    const result = await pool.query(query, [company_id, ...vendedoresIds]);
    return result.rows;
  }

  // Buscar cliente por ID (filtrado por company_id)
  static async findById(id, company_id, vendedorId = null) {
    let query = `
      SELECT c.*, u.nome as vendedor_nome
      FROM clientes c
      LEFT JOIN usuarios u ON c.vendedor_id = u.id
  ...
  // Atualizar cliente (filtrado por company_id)
  static async update(id, clienteData, company_id, vendedorId = null) {
    const {
      // Dados Básicos
      nome,
      cpf,
  ...
  // Atualizar apenas a etapa (para o Kanban, filtrado por company_id)
  static async updateEtapa(id, etapa, company_id, vendedorId = null) {
    let query = 'UPDATE clientes SET etapa = $1 WHERE id = $2 AND company_id = $3';
    const values = [etapa, id, company_id];
  ...
  // Deletar cliente (filtrado por company_id)
  static async delete(id, company_id, vendedorId = null) {
  ...
  // Estatísticas por etapa (filtrado por company_id)
  static async estatisticasPorEtapa(company_id, vendedorId = null) {
    let query = `
