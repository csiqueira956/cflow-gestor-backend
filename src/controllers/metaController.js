...
export const listarMetas = async (req, res) => {
  try {
    ...
    const metas = await Meta.list(company_id, filtros);
    res.json({ metas });
  } catch (error) {
...
export const buscarMeta = async (req, res) => {
  try {
    const { id } = req.params;
    const { company_id } = req.user;
    const meta = await Meta.findById(id, company_id);
...
export const atualizarMeta = async (req, res) => {
  try {
    ...
    const metaExistente = await Meta.findById(id, company_id);
    if (!metaExistente) {
      return res.status(404).json({ error: 'Meta não encontrada' });
    }
...
export const deletarMeta = async (req, res) => {
  try {
    ...
    await Meta.delete(id, company_id);
