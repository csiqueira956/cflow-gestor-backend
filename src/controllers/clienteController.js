...
export const listarClientes = async (req, res) => {
  try {
    ...
    if (role === 'vendedor') {
      ...
      clientes = await Cliente.list(company_id, userId);
      ...
      const vendedoresEquipe = await Usuario.findVendedoresByEquipeId(equipe_id, company_id);
      const vendedoresIds = vendedoresEquipe.map(v => v.id);

      // Buscar clientes de todos os vendedores da equipe
      clientes = await Cliente.listByVendedores(company_id, vendedoresIds);
    } else {
      // Admin vê todos os clientes
      clientes = await Cliente.list(company_id, null);
    }
...
export const buscarCliente = async (req, res) => {
  try {
    ...
    const cliente = await Cliente.findById(id, company_id, vendedorIdFiltro);
...
export const atualizarCliente = async (req, res) => {
  try {
    ...
    const clienteAtualizado = await Cliente.update(id, clienteData, company_id, vendedorIdFiltro);
...
export const atualizarEtapa = async (req, res) => {
  try {
    ...
    const clienteAtualizado = await Cliente.updateEtapa(id, etapa, company_id, vendedorIdFiltro);
...
export const deletarCliente = async (req, res) => {
  try {
    ...
    const clienteDeletado = await Cliente.delete(id, company_id, vendedorIdFiltro);
...
export const estatisticas = async (req, res) => {
  try {
    ...
    const estatisticas = await Cliente.estatisticasPorEtapa(company_id, vendedorIdFiltro);
...
export const criarClientePublico = async (req, res) => {
  try {
    ...
    console.log('💾 Criando cliente com dados:', {
      ...
      company_id: clienteData.company_id
    });
