...
export const estatisticasDashboard = async (req, res) => {
  try {
    ...
    `;
    const metaResult = await pool.query(metaQuery, [mesAtual, company_id]);
    ...
    if (role === 'vendedor') {
      ...
      `;
      vendasParams = [mesAtual, company_id, company_id, company_id, equipe_id, company_id];
    ...
    if (role === 'vendedor') {
      ...
      `;
      rankingParams = [company_id, req.user.id, company_id];
    } else if (role === 'gerente') {
      ...
      rankingQuery = `
        ...
        LIMIT 10
      `;
      rankingParams = [company_id, equipe_id, company_id];
    } else {
      ...
      rankingQuery = `
        ...
        LIMIT 10
      `;
      rankingParams = [company_id, company_id, company_id];
    }
